...
# Gagamitin natin ang global direct URL routing engine ng Our Daily Bread
BASE_URL = "https://odb.org"
# ===========================================
...
def get_target_date_url():
    """Generates the static target URL based on current Philippine Time calendar numbers."""
    # Kumuha ng exact date ngayon sa Philippine Time (UTC+8)
    now_ph = datetime.datetime.utcnow() + datetime.timedelta(hours=8)
    
    year = now_ph.strftime("%Y")   # e.g., "2026"
    month = now_ph.strftime("%m")  # e.g., "07"
    day = now_ph.strftime("%d")    # e.g., "05"
    
    # Bumuo ng permanenteng link structure ng global daily readings
    direct_url = f"{BASE_URL}/{year}/{month}/{day}"
    print(f"🎯 Generated Direct Calendar URL: {direct_url}")
    return direct_url

def fetch_devotion_content(url):
    """Scrapes Title, Scripture, and Devotion Text from the daily archive layout."""
    try:
        html = fetch_html_with_playwright(url)
        soup = BeautifulSoup(html, "html.parser")

        # Mas pinatibay na selectors para sa standard structural template ng ODB
        title_tag = soup.find("h1") or soup.find("h2", class_=lambda c: c and "title" in c.lower()) or soup.find("h1", class_=lambda c: c and "devotional" in c.lower())
        title = title_tag.get_text(strip=True) if title_tag else "Daily Devotion"

        # Scripture Reference Tracking
        scripture = ""
        scripture_btn = (
            soup.find("button", attrs={"aria-label": lambda a: a and ":" in a}) or 
            soup.find("button", string=lambda t: t and ":" in t) or
            soup.find("div", class_=lambda c: c and "verse" in c.lower()) or
            soup.find("span", class_=lambda c: c and "verse" in c.lower())
        )
        if scripture_btn:
            scripture = scripture_btn.get_text(strip=True)
        
        # Extract Main Post Reading Content Blocks
        paragraphs = []
        all_p = soup.find_all("p")
        for p in all_p:
            text = p.get_text(strip=True)
            # Tanging mga totoong talata lang ang kunin; i-bypass ang ads, links, at copyright markers
            if len(text) > 40 and not any(word in text.lower() for word in ["copyright", "all rights reserved", "sign up", "subscribe"]):
...
def generate_soap_format(title, scripture, content):
    ...
    if not content or content.startswith("Error") or len(content) < 100:
        return "Can't process the devotion text. Looks like something went wrong with the scraper layout."
...
if __name__ == "__main__":
    print("🔍 Generating today's direct date URL...")
    ...
    print("✅ Finished script!")import os
import datetime
import requests
from bs4 import BeautifulSoup
from google import genai
